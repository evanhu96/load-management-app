...
class LoadDataCollector {
  constructor(config) {
    this.config = {
      serverUrl: config.serverUrl || process.env.SERVER_URL || 'http://localhost:3001',
  ...
  async setupFileWatchers() {
    try {
      ...
      this.log('info', `File watcher setup complete. Watching ${validPaths.length} paths.`);
    } catch (error) {
  ...
  async processFile(filePath) {
    try {
      ...
      if (lastProcessed && lastProcessed >= lastModified) {
        this.log('debug', `Skipping ${filePath} - no changes since last processing`);
        return;
      }
      ...
      if (loads.length > 0) {
        ...
        this.log('info', `Successfully processed ${loads.length} loads from ${path.basename(filePath)}`);
      } else {
    ...
    for (const [key, load] of entries) {
      try {
        ...
        const normalizedLoad = {
          hash: load.hash || key || `${fileName}-${Date.now()}-${Math.random()}`,
  ...
  validateLoad(load) {
    ...
    for (const field of required) {
      if (!load[field]) {
        this.log('warn', `Load missing required field: ${field}`, { hash: load.hash });
        return false;
      }
    }
    
    if (load.rate <= 0) {
      this.log('warn', `Load has invalid rate: ${load.rate}`, { hash: load.hash });
      return false;
    }
    
    if (load.truck !== 1 && load.truck !== 2) {
      this.log('warn', `Load has invalid truck: ${load.truck}`, { hash: load.hash });
      return false;
    }
  ...
  handleConnectionError(error) {
    ...
    if (this.retryCount < this.config.maxRetries) {
      this.log('warn', `Retrying connection in ${this.config.retryInterval}ms (attempt ${this.retryCount}/${this.config.maxRetries})`);
      setTimeout(() => this.initialize(), this.config.retryInterval);
    } else {
      this.log('error', 'Max retry attempts reached. Please check your configuration and network connection.');
    }
  }
...
class CollectorCLI {
  ...
  showStatus() {
    ...
    console.log(`Uptime: ${status.uptime ? Math.round(status.uptime / 1000) : 0}s`);
  }
  ...
  async testConnection() {
    ...
    try {
      const response = await fetch(`${this.collector.config.serverUrl}/api/health`);
      if (response.ok) {
